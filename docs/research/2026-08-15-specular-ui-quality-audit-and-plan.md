...
12. MagicPath is an exploratory and supplemental resource rather than a second implementation system. After implementation, production components, Storybook stories, and approved visual baselines are operationally authoritative; routine two-way synchronization is unnecessary.
13. Minor enhancements must improve an existing task, remain local and reversible, require no schema, permission, or API change, fit inside the current ticket's tests, and introduce no new workflow concept.
14. Owner visual review occurs at three lightweight checkpoints: after foundations and highest-risk states, after the core authoring surface, and after secondary surfaces plus final evidence.
15. No active hosted state remains silently unfinished. Any exception is exact, owned, dated, expiring, tracked, and accepted by the owner.
16. Live cutover requires green required CI, complete synthetic visual evidence, critical WebKit checks, supplemental iPhone Simulator receipts, final owner preview approval, and an identified rollback revision. Owner greenlight is the sole publication trigger.
