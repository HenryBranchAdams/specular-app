...
|---|---|---|---|
| 1 | Installable mobile PWA creates, reloads, and retains a local thread | Immutable-server verification requires the built manifest and service worker; `installed service worker keeps the PWA shell available offline`; `first run, delayed persistence, reload, Challenge, conclusion, and Keep digging` on Chromium/WebKit at 320/375/430 | Pass |
...
| 8 | Spectral glass meets performance, contrast, reduced-motion, and target requirements | Lighthouse performance 0.99/accessibility 1.00; axe across empty/thread/Challenge/conclusion/capsule/offline/voice; scripted long-task test; 44px computed targets; visible-focus style assertion; nonzero safe-area-variable browser check; 320–430 matrix | Pass for automated release gate; physical release evidence follows `tests/e2e/browser-compatibility.md` |
...
|---|---|---|
| TypeScript, lint, unit/component/contract, client/server builds | `npm run validate` on Node 22.23.1/npm 10.9.8 — 27 files / 368 tests | Pass |
...
| Lighthouse | `npm run lighthouse`; Lighthouse 12.6.1, performance 0.99, accessibility 1.00, best practices 1.00, SEO 1.00, console errors 1.00 | Pass |
| Scripted interaction long tasks | `scripted mobile interactions produce no task longer than 50 milliseconds` | Pass |
| Immutable runtime/security/privacy | `node scripts/verify-production.mjs` — ten checks, including same-origin static assets, manifest, and service worker | Pass |
| Integrated render smoke | Built server at a fresh local origin; desktop 1280 × 800 and mobile 375 × 812; composer enablement, capsule open/close, zero horizontal overflow, clean console | Pass |
| Container build and health | `docker build --target production -t specular:development-ready .`; image `sha256:9173f31fb0430f9ac6a37406e4a54977bbdf16868c4b63044b5f3f7ff9740ad9`; non-root `node`; Docker health `healthy`; PWA and `/healthz` HTTP 200 | Pass |
| Compose definitions | Compose 5.1.4 `--profile development --profile preview --profile production config --quiet` | Pass |
...
- No live OpenAI request was made. A local key is provisioned for development, but the production adapter and qualitative live smoke remain explicit, billable release actions under specification §14.
